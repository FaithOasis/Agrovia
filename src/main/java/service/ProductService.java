...
public class ProductService {
    ...
    public ProductResponse createProduct(ProductRequest request) {
        ...
        if (request.getCategoryId() != null) {
            PostCategory category = mapToPostCategory(request.getCategoryId());
    ...
    private ProductResponse mapToProductResponse(Product product) {
        ...
        // Map category - FIXED: Using the correct CategorySummary constructor
        if (product.getCategory() != null) {
            // Get the enum name as category name
            String categoryName = product.getCategory().name();

            // Use ordinal for ID (0-based index)
            Long categoryId = (long) product.getCategory().ordinal();

            // Using the constructor with (Long, String) parameters
            CategorySummary categorySummary = new CategorySummary(categoryId, categoryName);
    ...
    // Helper method to map category ID to PostCategory enum
    private PostCategory mapToPostCategory(Long categoryId) {
        if (categoryId == null) return null;

        // This maps based on ordinal position
